...
export async function POST(req: Request) {
  const maxRetries = 2;
  let attempt = 0;
  const retryDelayMs = Number(process.env.DID_STREAM_RETRY_DELAY_MS ?? 1500);
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (res.status === 403 && attempt < maxRetries) {
          console.warn(`[API] Max sessions reached. Retrying in ${retryDelayMs}ms... (Attempt ${attempt + 1}/${maxRetries})`);
